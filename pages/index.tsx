...
export default function Home() {
    ...
    const tipo = localStorage.getItem('tipo');
     ...
     if (isLoggedIn === 'true' && userId) {
        if(tipo === 'administrador') {
          router.push('/administrador').then(() => setLoading(false));;
        } else  if(tipo === 'usuarioTitulado') {
          router.push('/perfil').then(() => setLoading(false));;
        }
      
    } else {
      setLoading(false); // No está logueado, detener la carga
    }
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    .then(result => {
       ...
       if (result.data.loginStatus && result.data.result[0].estado === 1) {
        if(result.data.result[0].activo === 0)
          {
          ...
          } else 
          {
            if (result.data.result[0].administrador) {
              ...
              localStorage.setItem('tipo', result.data.role);
          ...
          router.push(`/registro/datosAdicionales?id=${result.data.result[0].id}`)
  ...
  return (
    <body className="md:bg-gradient-to-r md:from-[#c91313a6] md:from-10% md:via-[#9d6cd2] md:via-30% md:to-[#2e14a3a8] to-90% flex items-center justify-center">
      <div className="max-w-[500px] md:max-w-[960px] grid grid-cols-2 items-center p-5 rounded-2xl md:bg-sky-800 gap-20">
        <div className="relative bg-[#6363637c] hidden md:flex">
        ...
        <div className="max-w-56 md:max-w-80 grid gap-5">
          ...
          >
            <div className="relative">
              ...
              <input 
                  ...
                  name="email"
                  placeholder="Usuario" 
                  className="w-52 md:w-80 bg-cyan-600 py-2 px-12 rounded-full focus:bg-fondo2 focus:outline-none focus:ring focus:ring-azulNeon focus:drop-shadow-lg placeholder:text-white"
            ...
            <button 
              className="bg-gradient-to-r from-blue-900 to-cyan-700 w-52 md:w-80 font-semibold rounded-full py-2 group relative inline-flex h-12 items-center justify-center overflow-hidden bg-neutral-950 px-6 text-neutral-200"
              name='submit'
            >
              <span>Ingresar</span>
